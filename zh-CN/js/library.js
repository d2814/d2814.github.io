...
function CodeTheme(){
	...
	this.cssList = ["prism.min.css","prism-dark.min.css","prism-funky.min.css","prism-okaidia.min.css","prism-twilight.min.css","prism-coy.min.css","prism-solarizedlight.min.css","prism-tomorrow.min.css"];
...
function SnackbarUtil(){
	...
	this.on = function(obj,pos){
		...
		mdui.snackbar({message: msg,position: pos});
	}
}
...
function Catalogue(){
	...
	this.toggle = function(){
		if(this.catalogue.attr("stauts") != "show"){
			this.show();
			let data = [{lang:"zh-CN",msg:"双击此按钮切换主菜单！"},
						{lang:"en-US",msg:"Double-click the button to switch to main menu!"}];
			let snackbarUtil = new SnackbarUtil();
			snackbarUtil.on(data,"top");
		}else{
			this.hide();
			let data = [{lang:"zh-CN",msg:"双击此按钮切换文章目录！"},
						{lang:"en-US",msg:"Double-click the button to switch to article directory!"}];
			let snackbarUtil = new SnackbarUtil();
			snackbarUtil.on(data,"top");
		}
	}
